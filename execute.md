...
- [x] Phase 9: Leaderboard — ตำแหน่งผู้ใช้เมื่ออยู่นอก top 20 — `GET /api/leaderboard?forCandidate=…` + `yourEntry` / `inTopList`; Pinia `leaderboardYourEntry`; แถบ “Your position” ใน `LeaderboardView`
...
| 9 | `GET /api/leaderboard` + `forCandidate`; `exam_result_gorm.CandidateRank`; `LeaderboardView` แสดงแถบเมื่อ `yourEntry.inTopList === false`; ดู [`docs/api.md`](./docs/api.md) |
...
- API: `http://localhost:8080` — `GET /api/questions`, `POST /api/submit`, `GET /api/leaderboard` (optional `forCandidate` สำหรับ `yourEntry`)
